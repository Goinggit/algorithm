...
struct my_node *my_search(struct rb_root *root, Type key)
{
    ...
    while (rbnode!=NULL)
    {
        struct my_node *mynode = rb_container_of(rbnode, struct my_node, rb_node);
...
int my_insert(struct rb_root *root, Type key)
{
    ...
    while (*tmp)
    {
        struct my_node *my = rb_container_of(*tmp, struct my_node, rb_node);
...
void main()
{
    int a[] = {10, 40, 30, 60, 90, 70, 20, 50, 80, 100, 110, 120};
    int i, ilen=LENGTH(a);
